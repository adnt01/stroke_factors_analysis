...
## Necessary Packages
The "Linear regression, classification, and resampling" module uses its own isolated environment called `team-project` so that packages don’t conflict with other projects. 
...
1. Create a virtual environment called `team-project`:
    ```
    uv venv team-project --python 3.11
    ```

2. Activate the environment:
    - for macOS/Linux:
        ```
        source team-project/bin/activate
        ```
        
    - for windows (git bash):    
        ```
        source team-project/Scripts/activate
        ```
